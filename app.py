...
# ───────────────────────────────────
# kapasite > 0 olanlarda atık/kapasite, diğerlerinde 0
valid = df["total_capacity_kg"] > 0
df["util_ratio"] = 0.0
df.loc[valid, "util_ratio"] = (
    df.loc[valid, "total_kg"] / df.loc[valid, "total_capacity_kg"]
)
# 0–1 sınırı
df["util_ratio"] = df["util_ratio"].clip(lower=0, upper=1)
